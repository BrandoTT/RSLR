...
from dataloader import cifar_stl_noise_loader as loader
import torch.nn as nn
from tensorboardX import SummaryWriter
import json

parser = argparse.ArgumentParser(description=' -- NoiseUDA CIFAR STL -- ')
parser.add_argument('--lr', default=0.01, type=float)
parser.add_argument('--batch_size_s', default=64)
parser.add_argument('--batch_size_t', default=64)
parser.add_argument('--image_size', default=32, type=int)
parser.add_argument('--Epoch', default=50, type=int)
parser.add_argument('--Noise', default='sym', type=str, choices=['asym','sym'])
parser.add_argument('--dset', default='cifar_stl', type=str)
...
parser.add_argument('--corruption_level', default=0.1, type=float, help='Noise level')
parser.add_argument('--noise_ratio', default=0.4, type=float, help='Proportion of noise data')
parser.add_argument('--num_class', default=9, type=int, help='Class number')
...
print("===========  Training Start(CIFAR and STL{}) Noise ratio:{}===========\n".format(args.dset,args.noise_ratio))
'''2021-11-30'''
...
if args.phase == 'clean':
    ...
    if args.Log=='yes':
        print(args.Log)
        log_name = 'CIFARandSTL_{}2{}_(New Papaer_ResNet50)Noise_{}'.format(source_name,target_name,args.noise_ratio)
        parse = record_config(dir_path,log_name,args.dset)
        parse.make_dir()
        writer = SummaryWriter(parse.log_root)
   
    Source_loader = loader.cifar_stl_Noisy(root=root,
                                            ...
                                            noise_file='%s/%s_%.1f_%s.json'%(root+'{}/{}/Noise_labels/'.format(source_name,source_name),source_name,args.noise_ratio, 'sym')
    )
    target_noise_ratio = 0.0

    Target_loader = loader.cifar_stl_Noisy(root=root,
                                            ratio=target_noise_ratio,
                                            level=target_noise_ratio,
                                            type=args.Noise,
                                            dataset_name=target_name,
                                            batch_size=args.batch_size_t, #8
                                            img_size=args.image_size,
                                            noise_file='%s/%s_%.1f_%s.json'%(root+'{}/{}/Noise_labels/'.format(target_name,target_name),target_name,target_noise_ratio, 'sym')
    )
    # 拿到 Target的class ratio
    print("Loader Completed!\n")
    #定义office网络
    Feature_L,Class_1,Class_2,CNN_TarNet = creat_model(model='cifar_stl',args=args)
    # optimizer
    ...
    #################
    # source 
    Source_Dataset = Source_loader.run('ALL_Data') # return weak_img, strong_img, targets, index
    print("length of source:%d"%len(Source_Dataset))
    # target 
    Target_Dataset = Target_loader.run('ALL_Data') # return weak_img, strong_img, targets, index
    print("length of target:%d"%len(Target_Dataset))
    # test data
    length_tar,test_Target = Target_loader.run('test')
    length_tar,test_Source = Source_loader.run('test')
    ...
    for epoch in range(args.Epoch):
       ...
       #all_loss = [[],[]]
        ...
        AccTarget = SNetTest(Feature_L,CNN_TarNet,test_Target)
    ...
    json.dump(NoiseCleanRatio_1, open(dir_path+'/Record/cifar_stl/{}/Noisecheck1.json'.format(args.RecordFolder), "w"))
    json.dump(NoiseCleanRatio_2, open(dir_path+'/Record/cifar_stl/{}/Noisecheck2.json'.format(args.RecordFolder), "w"))
    json.dump(Recall_source1, open(dir_path+'/Record/cifar_stl/{}/recall_s1.json'.format(args.RecordFolder), "w"))
    json.dump(Precision_source1, open(dir_path+'/Record/cifar_stl/{}/pre_s1.json'.format(args.RecordFolder), "w"))
    json.dump(Recall_source2, open(dir_path+'/Record/cifar_stl/{}/recall_s2.json'.format(args.RecordFolder), "w"))
    json.dump(Precision_source2, open(dir_path+'/Record/cifar_stl/{}/pre_s2.json'.format(args.RecordFolder), "w"))
    json.dump(Recall_target, open(dir_path+'/Record/cifar_stl/{}/recall_t.json'.format(args.RecordFolder), "w"))
    json.dump(Precision_target, open(dir_path+'/Record/cifar_stl/{}/pre_t.json'.format(args.RecordFolder), "w"))
    json.dump(Acc_t, open(dir_path+'/Record/cifar_stl/{}/Acc_t.json'.format(args.RecordFolder), "w"))
    json.dump(Acc_s1, open(dir_path+'/Record/cifar_stl/{}/Acc_s1.json'.format(args.RecordFolder), "w"))
    json.dump(Acc_s2, open(dir_path+'/Record/cifar_stl/{}/Acc_s2.json'.format(args.RecordFolder), "w"))
    json.dump(ImbalanceRatio_t, open(dir_path+'/Record/cifar_stl/{}/ImbalanceRatio_t.json'.format(args.RecordFolder), "w"))
    json.dump(ImbalanceRatio_s1, open(dir_path+'/Record/cifar_stl/{}/ImbalanceRatio_s1.json'.format(args.RecordFolder), "w"))
    json.dump(ImbalanceRatio_s2, open(dir_path+'/Record/cifar_stl/{}/ImbalanceRatio_s2.json'.format(args.RecordFolder), "w"))
    json.dump(ImbalanceRatio_s1_l, open(dir_path+'/Record/cifar_stl/{}/ImbalanceRatio_s1_l.json'.format(args.RecordFolder), "w"))
    json.dump(ImbalanceRatio_s2_l, open(dir_path+'/Record/cifar_stl/{}/ImbalanceRatio_s2_l.json'.format(args.RecordFolder), "w"))
    print("\n")
