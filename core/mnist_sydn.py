...
from dataloader import syndigits_noise_loader as loader_target
...
parser.add_argument('--dset', default='M2Syn', type=str, choices=['S2M','M2m', 'Syn2SVHN', 'Syn2M', 'M2Syn'])
...
print("===========  Training Start(MNIST-->SYDN) Noise(ratio:{},type:{})  ===========\n".format(args.noise_ratio,args.Noise))
...
if args.phase == 'clean': 
    ...
    # Log
    if args.Log=='yes':
        log_name = 'MNIST2SYDN_Noise_(New_Paper_data){}_{}'.format(args.Noise,args.noise_ratio)
    ...
    loader_source = loader_source.mnist_dataloader(root=root,
                                                        ...
                                                        noise_file='%s/%.2f_%s.json'%(root+'/MNIST_NoiseLabels',args.noise_ratio,args.Noise)
    )
    target_noise_ratio = 0.0
    # Syndigits -- Target
    loader_target = loader_target.syndigits_dataloader(root=root,ratio=target_noise_ratio,level=0.0,type=args.Noise,batch_size=args.batch_size_t,img_size=args.image_size,
                                                    noise_file='%s/%.1f_%s.json'%(root+'/SynDigits_NoiseLabels',target_noise_ratio,'sym')
    )
